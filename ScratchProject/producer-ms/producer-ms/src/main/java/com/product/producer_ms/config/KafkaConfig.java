package com.product.producer_ms.config;
...
//idepotent to work inflightRequest should not be greater than 5
        return config;
    }

    @Bean
    ProducerFactory<String, ProductModel> producerFactory() {
        return new DefaultKafkaProducerFactory<>(producerConfigs());
    }

    @Bean
    KafkaTemplate<String, ProductModel> kafkaTemplate() {
        return new KafkaTemplate<String, ProductModel>(producerFactory());
    }

    @Bean
    public NewTopic createTopic()
    {
       return TopicBuilder.name(PRODUCT_ORDER_MS).
